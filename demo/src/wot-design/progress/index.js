  ...
  methods: {
    ...
    update(targetPercent, showColor) {
      ...
      const {
        ...
        duration
      } = this.data; // transition-duration的优先更高
    ...
    controlProgress() {
      ...
      if (isObjArray && colorArray.some(({
        percentage
      }) => Number.isNaN(parseInt(percentage)))) {
      ...
      showPercent > percentage // 减小不加动画，找到第一个比target大的锚点，取锚点颜色并设置target值
      ? partList.some(part => {
