  ...
  methods: {
    ...
    observerContentScroll() {
      const {
        ...
        height,
        width
      } = this.data; // 视图在 render tree 中未呈现，吸顶无任何意义。
      ...
      this.getRect('.wd-sticky').then(res => {
        ...
        if (res.bottom <= offset) this.scrollHandler({
    ...
    scrollHandler({
      ...
      const {
        offsetTop,
        height
      } = this.data; // sticky 高度大于或等于 wd-sticky-box，使用 wd-sticky-box 无任何意义
      ...
      if (boundingClientRect.top <= offsetTop) {
        this.data.state = 'sticky'; // 开始吸顶，固定到顶部
        ...
        this.data.state = 'normal'; // 完全展示，结束吸顶
