  ...
  relations: {
    '../checkbox/index': {
      ...
      linked(target) {
        ...
        const index = this.children.indexOf(target); // 如果当前子节点为第一个组件，将其isFirst设置为true
        ...
        if (index === this.children.length - 1) {
          ...
          prevChild && renderData(prevChild, {
            isLast: false
          });
        }
      },
      ...
      unlinked(target) {
        ...
        if (this.children.length === 0) return; // 如果当前删除的组件为第一个组件，将删除后的第一个组件的isFirst设置为true
