...
VueComponent({
  ...
  methods: {
     ...
     */
    getWidths() {
      return Promise.all([this.getRect('.wd-swipe-action__left').then(rects => {
    ...
    swipeMove(offset = 0) {
      this.offset = offset;
      const transform = `translate3d(${offset}px, 0, 0)`; // 跟随手指滑动，不需要动画
     ...
     */
    endDrag() {
      if (this.data.disabled) return; // 滑出"操作按钮"的阈值
  ...
  beforeCreate() {
    // 滑动开始时，wrapper的偏移量
    this.originOffset = 0; // wrapper现在的偏移量

    this.wrapperOffset = 0; // 是否处于滑动状态

    this.touching = false; // 设置默认 beforeClose 函数
