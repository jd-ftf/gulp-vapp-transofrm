...
import pickerViewProps from '../pickerView/props'; // 本地时间戳
...
VueComponent({
  ...
  props: {
    value: {
      ...
      observer(val, oldVal) {
        if (val === oldVal) return; // 外部传入值更改时 更新picker数据
  ...
  methods: {
    ...
    updateColumnValue(value) {
      const values = this.getPickerValue(value, this.data.type); // 更新pickerView的value,columns
    ...
    updateInnerValue() {
      ...
      const year = values[0] && parseInt(values[0]); // 处理月 索引位1

      const month = values[1] && parseInt(values[1]);
      const maxDate = getMonthEndDay(year, month); // 处理 date 日期 索引位2
