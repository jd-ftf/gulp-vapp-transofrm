...
VueComponent({
  externalClasses: ['custom-textarea-class', 'custom-input-class', 'custom-label-class'],
  ...
  methods: {
    ...
    clear() {
      ...
      this.requestAnimationFrame().then(() => this.requestAnimationFrame()).then(() => this.requestAnimationFrame()).then(() => {
        this.setData({
          focus: true
        }, () => {
          this.$emit('clear');
