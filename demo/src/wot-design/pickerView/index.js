...
VueComponent({
  props: {
    ...
    columns: {
      ...
      observer(columns) {
        ...
        this.setData({
          formatColumns: this.formatArray(columns)
        });
        /**
         * 每次改变都要重置选中项
         * 1.选中每列的第一个
  ...
  methods: {
    ...
    selectWithValue(value) {
      if (this.data.columns.length === 0) return; // 使其默认选中首项
      ...
      if (type.indexOf(valueType) === -1) throw Error(`value must be one of ${type.toString()}`); // 在props初始化的时候有可能会调用此函数，此时需要保证formatColumns已经被设置，关于此问题更多详情参考/ISSUE.md。
     ...
     */
    formatArray(array) {
      array = array instanceof Array ? array : [array]; // 检测第一层的type
    ...
    getChangeDiff(value) {
      ...
      value = value.slice(0, this.data.formatColumns.length); // 保留选中前的

      const origin = this.data.selectedIndex.slice(0); // 存储赋值旧值，便于外部比较

      let selectedIndex = this.data.selectedIndex; // 开始应用最新的值
      ...
      this.preSelectedIndex = origin; // diff出变化的列
      // const diffCol = selectedIndex.findIndex((row, index) => row !== origin[index])

      const diffCol = this.getChangeIndex(selectedIndex, origin);
      if (diffCol === -1) return; // 获取变化的的行

      const diffRow = selectedIndex[diffCol]; // 如果selectedIndex只有一列，返回选中项的索引；如果是多项，返回选中项所在的列。
    ...
    getSelects() {
      ...
      const selects = selectedIndex.map((row, col) => formatColumns[col][row]); // 单列选择器，则返回单项
  ...
  created() {
    ...
    if ((!this.data.value || !this.data.value.length) && this.data.columns.length !== 0) {
