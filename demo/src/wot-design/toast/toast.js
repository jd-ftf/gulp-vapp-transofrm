...
const Toast = toastOptions => {
  // 覆盖模板中的选项
  const options = Object.assign({}, defaultOptions, parseOptions(toastOptions)); // 获取页面栈中栈顶页面(当前显示的页面)
  ...
  delete options.selector; // 强制清除队列中所有的toast
  ...
  toast.setData(options); // 将toast放入队列

  queue.push(toast); // 返回toast实例
...
Toast.error = createMethod({
  iconName: 'error'
});
Toast.warning = createMethod({
  iconName: 'warning'
});
