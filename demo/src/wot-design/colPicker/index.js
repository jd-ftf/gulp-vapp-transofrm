  ...
  methods: {
    ...
    setLineStyle(animation = true) {
      ...
      this.getRect($item, true).then(rects => {
        const rect = rects[currentCol]; // const width = lineWidth || (slidableNum < items.length ? rect.width : (rect.width - 14))
    ...
    lineScrollIntoView() {
      ...
      Promise.all([this.getRect($item, true), this.getRect($container)]).then(([navItemsRects, navRect]) => {
        if (navItemsRects.length === 0) return; // 选中元素

        const selectItem = navItemsRects[currentCol]; // 选中元素之前的节点的宽度总和

        const offsetLeft = navItemsRects.slice(0, currentCol).reduce((prev, curr) => prev + curr.width, 0); // scroll-view滑动到selectItem的偏移量
