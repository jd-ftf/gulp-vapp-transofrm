...
VueComponent({
  ...
  methods: {
    init() {
      ...
      const width = column ? 100 / column + '%' : 100 / children.length + '%'; // 单独定义间隔

      const gutterStyle = gutter ? `padding:${gutter}px ${gutter}px 0 0; background-color: transparent;` : ''; // 单独定义正方形

      const squareStyle = square ? `background-color:transparent; padding-bottom: 0; padding-top:${width}` : ''; // 间隔+正方形
