...
VueComponent({
  ...
  relations: {
    '../tab/index': {
      ...
      linked(child) {
        this.children = this.children || []; // 在建立relations时tabs保存tab实例中的name的唯一性。
  ...
  methods: {
    ...
    updateLineStyle(animation = true) {
      if (!this.inited) return; // const { activeIndex, lineWidth, lineHeight, slidableNum, items } = this.data
      ...
      this.getRect($item, true).then(rects => {
        const rect = rects[activeIndex]; // const width = lineWidth || (slidableNum < items.length ? rect.width : (rect.width - 14))
        ...
        this.data.lineStyle !== lineStyle && this.setData({
          lineStyle
        });
      });
    },
     ...
     */
    setActiveTab(animation = this.data.animated, initBody = false) {
      ...
      this.getRect($body).then(rect => {
        ...
        lazyRender && !items[activeIndex].painted && this.children[activeIndex].setData({
          painted: true
        }); // 初始化时会依次设置child的宽度

        initBody && this.children && this.children.length > 0 && this.children.forEach(child => {
          child.setData(lazyRender ? {
            width
          } : {
            width,
            painted: true
    ...
    scrollIntoView() {
      ...
      Promise.all([this.getRect($item, true), this.getRect($container)]).then(([navItemsRects, navRect]) => {
        ...
        const offsetLeft = navItemsRects.slice(0, activeIndex).reduce((prev, curr) => prev + curr.width, 0); // scroll-view滑动到selectItem的偏移量
  ...
  beforeCreate() {
    ...
    this.setActive = debounce(function (value = 0, init = false) {
      const {
        items
      } = this.data; // 没有tab子元素，不执行任何操作

      if (items.length === 0) return; // name代表的索引超过了items的边界，自动用0兜底
