  ...
  methods: {
    ...
    chooseFile() {
      const {
        ...
        sourceType,
        beforeUpload
      } = this.data; // 设置为只选择图片的时候使用 chooseImage 来实现
      ...
      if (accept === 'image') {
        ...
        chooseFile({
          ...
          // 成功选择初始化file
          let files = null;
          files = Array.prototype.slice.call(res.tempFiles); // 单选只有一个
          ...
          const mapFiles = files => {
            files.forEach(file => {
              file.size <= maxSize ? this.initFile(file) : this.$emit('oversize', {
                file
              });
            });
          }; // 上传前的钩子
