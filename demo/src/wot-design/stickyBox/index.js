...
VueComponent({
  ...
  methods: {
    ...
    observerForChild(child) {
      ...
      this.deleteObserver(child);
      const observer = this.createObserver(child); // 如果 wd-sticky 比 wd-sticky-box还大，"相对吸顶"无任何意义,此时强制吸顶元素回归其占位符
      ...
      this.getRect('.wd-sticky-box').then(res => {
        ...
        if (res.bottom <= offset) this.scrollHandler(child, {
    ...
    scrollHandler(child, {
      ...
      if (boundingClientRect.bottom <= offset) {
        ...
        if (child.data.state === 'normal') return; // 顶元素开始遮盖不住父元素了，将吸顶元素恢复到吸顶模式
