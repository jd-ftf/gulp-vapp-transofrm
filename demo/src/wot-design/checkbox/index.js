...
VueComponent({
  ...
  data: {
    ...
    cellBox: false,
    buttonBox: false // 以下内容用于解决父子组件样式隔离的问题 —— END
  ...
  methods: {
    ...
    checkDisabled() {
      ...
      if ( // max 生效时，group 已经选满，禁止其它节点再选中。
      max && value.length >= max && !this.data.isChecked || // min 生效时，group 选中的节点数量仅满足最小值，禁止取消已选中的节点。
      min && value.length <= min && this.data.isChecked || // 只要子节点自己要求 disabled，那就 disabled。
