  ...
  methods: {
    ...
    setShowValue(tab = false, isConfirm = false) {
      ...
      if (region) {
        ...
        this.setData({
          showValue: tab ? this.data.showValue : [value[0] || isConfirm ? this.defaultDisplayFormat(items) : '', value[1] || isConfirm ? this.defaultDisplayFormat(endItems) : ''],
    ...
    columnDisabledRules(isStart, columns, cIndex, value, currentValue, boundary) {
      ...
      const column = columns[cIndex]; // 根据当前选择年确认 ranges[0][0] minYear ranges[0][1] maxYear 以此类推
    ...
    customColumnFormatter(picker) {
      ...
      const {
        ...
        startSymbol,
        formatter
      } = picker.data; // 校准上下方picker的value值，与内部innerValue对应
      ...
      const boundary = startSymbol ? picker.getPickerValue(end, type) : picker.getPickerValue(start, type); // 获取当前picekr中的源列数组
